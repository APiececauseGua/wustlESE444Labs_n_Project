...
void loop() {
  ...
  if(get_average_moisture() < threshold && !alert_already_sent){ 
    Serial.println("triggered warning email");
  ...
  if(rtc.getHours() == 17 && rtc.getMinutes() == 30 && !time_hit){
    Serial.println("triggered daily email");
    send_email(get_temperature(), get_average_moisture(), get_light(), warning);
    alert_already_sent = false;
    time_hit=true;
  }

  if(rtc.getHours() == 17 && rtc.getMinutes() == 31) {
    time_hit=false;
  }
  
  if(!already_sent_to_cloud){
    ThingSpeak.setField(1,get_light());
    ThingSpeak.setField(3,get_temperature());
    ThingSpeak.setField(2,get_average_moisture());
